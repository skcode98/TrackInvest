...
function openUsableMoneyDetails() {
    const sheet = document.getElementById('usable-money-sheet');
    const scrim = document.getElementById('usable-money-scrim');
    const body = document.getElementById('usable-money-details-body');
    if (!sheet || !scrim || !body) return;

    const usable = calculateUsableMoney();
    const immediate = usable.immediate;
    const emergency = usable.emergency;
    const total = usable.total;
...
function closeUsableMoneyDetails() {
    const sheet = document.getElementById('usable-money-sheet');
    const scrim = document.getElementById('usable-money-scrim');
...
function updatePortfolioCalculations() {
    ...
    if (goalsList) {
        goalsList.innerHTML = db.goals.length === 0 ? `<div class="empty-state-premium"><span class="material-symbols-rounded">flag</span><div class="es-title">No Goals Set</div></div>` : db.goals.map(g => {
            ...
            return `<div class="goal-card"><div class="goal-header"><div class="goal-title">${escapeHtml(g.name)} ${linkTag}</div><div class="goal-amt">${formatMoney(savedAmt)} / ${formatMoney(g.target)}</div></div><div class="goal-track"><div class="goal-fill" style="width:${perc}%;"></div></div><div class="goal-footer" style="font-size:12px; color:var(--md-on-surface-variant);display:flex;align-items:center;justify-content:space-between;"><span>${perc.toFixed(1)}% Achieved</span><span>${forecastHtml}</span></div><div style="display:flex;gap:6px;margin-top:8px;"><button onclick="event.stopPropagation();openGoalSheet(this.dataset.gid)" data-gid="${g.id}" style="flex:1;padding:8px;border:none;border-radius:20px;background:var(--md-surface-container-high);color:var(--md-on-surface-variant);cursor:pointer;font-size:12px;font-weight:500;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:4px;" title="Edit Goal"><span class="material-symbols-rounded" style="font-size:16px;">edit</span> Edit</button>${isLinked ? `<button onclick="event.stopPropagation();showGoalMonthHistory(this.dataset.gid2)" data-gid2="${g.id}" style="flex:1;padding:8px;border:none;border-radius:20px;background:var(--md-surface-container-high);color:var(--md-on-surface-variant);cursor:pointer;font-size:12px;font-weight:500;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:4px;" title="Monthly History"><span class="material-symbols-rounded" style="font-size:16px;">history</span> History</button>` : ''}</div></div>`;
        }).join('');
    }
